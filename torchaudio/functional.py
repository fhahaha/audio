...
def spectrogram(waveform: Tensor, pad: int, window: Tensor, n_fft: int,
                hop_length: int, win_length: int, power: Optional[float],
    ...
    spec_f = torch.stft(waveform, n_fft, hop_length, win_length, window, True,
                        "reflect", False, True)
...
def griffinlim(specgram: Tensor, window: Tensor, n_fft: int, hop_length: int,
               win_length: int, power: float, normalized: bool, n_iter: int,
               momentum: float, length: Optional[int],
               rand_init: bool) -> Tensor:
...
def create_kaldi_fb_matrix(n_freqs: int, f_min: float, f_max: float,
                           n_mels: int, sample_rate: int) -> Tensor:
    r"""Create a frequency bin conversion matrix as same as kaldi.
    ...
    # freq bins
    # Equivalent filterbank construction by Librosa
    all_freqs = torch.linspace(0, sample_rate // 2, n_freqs + 1)[:-1]
    all_freqs_mel = 1127.0 * math.log(1.0 + (all_freqs / 700.0))

    # calculate mel freq bins
    # hertz to mel(f) is 1127. * math.log(1. + (f / 700.))
    m_min = 1127.0 * math.log(1.0 + (f_min / 700.0))
    m_max = 1127.0 * math.log(1.0 + (f_max / 700.0))
    m_pts = torch.linspace(m_min, m_max, n_mels + 2)
    mel_diff = (m_max - m_min) / (n_mels + 1)
    slopes = m_pts.unsqueeze(0) - all_freqs_mel.unsqueeze(1)
    # create overlapping triangles
    zero = torch.zeros(1)
    down_slopes = (-1.0 * slopes[:, :-2]) / mel_diff  # (n_freqs, n_mels)
    up_slopes = slopes[:, 2:] / mel_diff  # (n_freqs, n_mels)
    fb = torch.max(zero, torch.min(down_slopes, up_slopes))
...
def magphase(complex_tensor: Tensor,
             power: float = 1.0) -> Tuple[Tensor, Tensor]:
...
def phase_vocoder(complex_specgrams: Tensor, rate: float,
                  phase_advance: Tensor) -> Tensor:
    ...
    complex_specgrams_1 = complex_specgrams.index_select(
        -2, (time_steps + 1).long())
...
def lfilter(
    ...
    padded_waveform = torch.zeros(n_channel,
                                  n_sample_padded,
                                  dtype=dtype,
                                  device=device)
    padded_waveform[:, (n_order - 1):] = waveform
    padded_output_waveform = torch.zeros(n_channel,
                                         n_sample_padded,
                                         dtype=dtype,
                                         device=device)
    ...
    window_idxs = torch.arange(n_sample,
                               device=device).unsqueeze(0) + torch.arange(
                                   n_order, device=device).unsqueeze(1)
    ...
    for i_sample, o0 in enumerate(input_signal_windows.t()):
        windowed_output_signal = padded_output_waveform[:, i_sample:(i_sample +
                                                                     n_order)]
...
def biquad(waveform: Tensor, b0: float, b1: float, b2: float, a0: float,
           a1: float, a2: float) -> Tensor:
    ...
    output_waveform = lfilter(
        waveform, torch.tensor([a0, a1, a2], dtype=dtype, device=device),
...
def highpass_biquad(waveform: Tensor,
                    sample_rate: int,
                    cutoff_freq: float,
                    Q: float = 0.707) -> Tensor:
...
def lowpass_biquad(waveform: Tensor,
                   sample_rate: int,
                   cutoff_freq: float,
                   Q: float = 0.707) -> Tensor:
...
def allpass_biquad(waveform: Tensor,
                   sample_rate: int,
                   central_freq: float,
                   Q: float = 0.707) -> Tensor:
...
def bandpass_biquad(waveform: Tensor,
                    sample_rate: int,
                    central_freq: float,
                    Q: float = 0.707,
                    const_skirt_gain: bool = False) -> Tensor:
...
def bandreject_biquad(waveform: Tensor,
                      sample_rate: int,
                      central_freq: float,
                      Q: float = 0.707) -> Tensor:
...
def equalizer_biquad(waveform: Tensor,
                     sample_rate: int,
                     center_freq: float,
                     gain: float,
                     Q: float = 0.707) -> Tensor:
...
def band_biquad(waveform: Tensor,
                sample_rate: int,
                central_freq: float,
                Q: float = 0.707,
...
def treble_biquad(waveform: Tensor,
                  sample_rate: int,
                  gain: float,
...
def bass_biquad(waveform: Tensor,
                sample_rate: int,
                gain: float,
    ...
    return biquad(waveform, b0 / a0, b1 / a0, b2 / a0, a0 / a0, a1 / a0,
                  a2 / a0)
...
def deemph_biquad(waveform: Tensor, sample_rate: int) -> Tensor:
    ...
    w0 = 2 * math.pi * central_freq / sample_rate
    A = math.exp(gain / 40.0 * math.log(10))
    alpha = math.sin(w0) / 2 * math.sqrt((A + 1 / A) *
                                         (1 / width_slope - 1) + 2)
...
def contrast(waveform: Tensor, enhancement_amount: float = 75.) -> Tensor:
...
def dcshift(waveform: Tensor,
            shift: float,
            limiter_gain: Optional[float] = None) -> Tensor:
    ...
    if limiter_gain is not None and shift > 0:
        ...
        temp = (waveform[mask] -
                limiter_threshold) * limiter_gain / (1 - limiter_threshold)
        output_waveform[mask] = (temp + limiter_threshold +
                                 shift).clamp(max=limiter_threshold)
        ...
        temp = (waveform[mask] +
                limiter_threshold) * limiter_gain / (1 - limiter_threshold)
        output_waveform[mask] = (temp - limiter_threshold +
                                 shift).clamp(min=-limiter_threshold)
        output_waveform[~mask] = (waveform[~mask] + shift).clamp(min=-1, max=1)
    else:
...
def overdrive(waveform: Tensor,
              gain: float = 20,
              colour: float = 20) -> Tensor:
...
def phaser(waveform: Tensor,
    ...
    delay_buf = torch.zeros(waveform.shape[0],
                            delay_buf_len,
                            dtype=dtype,
                            device=device)
...
def _generate_wave_table(wave_type: str, data_type: str, table_size: int,
                         min: float, max: float, phase: float,
    ...
    if wave_type == 'SINE':
        d = (torch.sin(point.to(torch.float64) / table_size * 2 * math.pi) +
             1) / 2
...
def flanger(waveform: Tensor,
    ...
    if modulation not in ('sinusoidal', 'triangular'):
        raise ValueError(
            "Only 'sinusoidal' or 'triangular' modulation allowed")
    ...
    delay_bufs = torch.zeros(waveform.shape[0],
                             n_channels,
                             delay_buf_length,
                             dtype=dtype,
                             device=device)
    delay_last = torch.zeros(waveform.shape[0],
                             n_channels,
                             dtype=dtype,
                             device=device)
    ...
    for i in range(waveform.shape[-1]):
        ...
        delay_buf_pos = (delay_buf_pos + delay_buf_length -
                         1) % delay_buf_length

        cur_channel_phase = (channel_idxs * lfo_length * channel_phase +
                             .5).to(torch.int64)
        ...
        delayed_0 = delay_bufs[:, channel_idxs,
                               (delay_buf_pos + int_delay) % delay_buf_length]

        int_delay = int_delay + 1

        delayed_1 = delay_bufs[:, channel_idxs,
                               (delay_buf_pos + int_delay) % delay_buf_length]
        ...
        if interpolation == 'linear':
            ...
            delayed_2 = delay_bufs[:, channel_idxs,
                                   (delay_buf_pos + int_delay) %
                                   delay_buf_length]
        ...
        delay_last = delayed
        output_waveform[:, :,
                        i] = waveform[:, :, i] * in_gain + delayed * delay_gain
...
def mask_along_axis_iid(specgrams: Tensor, mask_param: int, mask_value: float,
                        axis: int) -> Tensor:
    r"""
    ...
    value = torch.rand(specgrams.shape[:2], device=device,
                       dtype=dtype) * mask_param
    min_value = torch.rand(specgrams.shape[:2], device=device,
                           dtype=dtype) * (specgrams.size(axis) - value)
    ...
    specgrams.masked_fill_((mask >= mask_start) & (mask < mask_end),
                           mask_value)
    specgrams = specgrams.transpose(axis, -1)
...
def mask_along_axis(specgram: Tensor, mask_param: int, mask_value: float,
                    axis: int) -> Tensor:
    r"""
    Apply a mask along ``axis``. Mask will be applied from indices ``[v_0, v_0 + v)``, where
    ``v`` is sampled from ``uniform(0, mask_param)``, and ``v_0`` from ``uniform(0, max_v - v)``.
...
def compute_deltas(specgram: Tensor,
    ...
    kernel = torch.arange(-n, n + 1, 1, device=device,
                          dtype=dtype).repeat(specgram.shape[1], 1, 1)

    output = torch.nn.functional.conv1d(
        specgram, kernel, groups=specgram.shape[1]) / denom
...
def _apply_probability_distribution(waveform: Tensor,
                                    density_function: str = "TPDF") -> Tensor:
    ...
    random_channel = int(torch.randint(channel_size, [
        1,
    ]).item()) if channel_size > 0 else 0
    random_time = int(torch.randint(time_size, [
        1,
    ]).item()) if time_size > 0 else 0
    ...
    if (density_function == "RPDF"):
        ...
        for ws in num_rand_variables * [time_size]:
            rand_chan = int(torch.randint(channel_size, [
                1,
            ]).item())
            gaussian += waveform[rand_chan][int(
                torch.randint(ws, [
                    1,
                ]).item())]
        ...
        TPDF = torch.bartlett_window(time_size + 1,
                                     dtype=signal_scaled.dtype,
                                     device=signal_scaled.device)
...
def dither(waveform: Tensor,
           density_function: str = "TPDF",
           noise_shaping: bool = False) -> Tensor:
...
def _compute_nccf(waveform: Tensor, sample_rate: int, frame_time: float,
                  freq_low: int) -> Tensor:
    r"""
    ...
    for lag in range(1, lags + 1):
        s1 = waveform[..., :-lag].unfold(-1, frame_size,
                                         frame_size)[..., :num_of_frames, :]
        s2 = waveform[..., lag:].unfold(-1, frame_size,
                                        frame_size)[..., :num_of_frames, :]

        output_frames = ((s1 * s2).sum(-1) /
...
def _combine_max(a: Tuple[Tensor, Tensor],
                 b: Tuple[Tensor, Tensor],
                 thresh: float = 0.99) -> Tuple[Tensor, Tensor]:
    """
...
def _find_max_per_frame(nccf: Tensor, sample_rate: int,
                        freq_high: int) -> Tensor:
    r"""
...
def sliding_window_cmn(
    ...
    cur_sumsq = torch.zeros(num_channels,
                            num_feats,
                            dtype=dtype,
                            device=device)
    cmn_waveform = torch.zeros(num_channels,
                               num_frames,
                               num_feats,
                               dtype=dtype,
                               device=device)
    for t in range(num_frames):
        ...
        if norm_vars:
            if window_frames == 1:
                cmn_waveform[:, t, :] = torch.zeros(num_channels,
                                                    num_feats,
                                                    dtype=dtype,
                                                    device=device)
            else:
    ...
    cmn_waveform = cmn_waveform.view(input_shape[:-2] +
...
def _measure(measure_len_ws: int, samples: Tensor, spectrum: Tensor,
             noise_spectrum: Tensor, spectrum_window: Tensor,
             spectrum_start: int, spectrum_end: int, cepstrum_window: Tensor,
             cepstrum_start: int, cepstrum_end: int,
             noise_reduction_amount: float, measure_smooth_time_mult: float,
             noise_up_time_mult: float, noise_down_time_mult: float,
             index_ns: int, boot_count: int) -> float:
    ...
    _index_ns = torch.tensor([index_ns] + [(index_ns + i) % samplesLen_ns
    ...
    noise_spectrum[spectrum_start:spectrum_end].mul_(_mult).add_(_d *
                                                                 (1 - _mult))
    _d = torch.sqrt(
        torch.max(
            _zeros, _d - noise_reduction_amount *
    ...
    result: float = float(
        torch.sum(
            complex_norm(_cepstrum_Buf[cepstrum_start:cepstrum_end],
                         power=2.0)))
    result = \
...
def vad(
    ...
    measure_smooth_time_mult = math.exp(-1. /
                                        (measure_smooth_time * measure_freq))
    ...
    while (pos < ilen and not has_triggered):
        ...
        for i in range(n_channels):
            ...
            if (measure_timer_ns == 0):
                ...
                mean_meas[i] = mean_meas[i] * trigger_meas_time_mult + meas * (
                    1. - trigger_meas_time_mult)

                has_triggered = has_triggered or (mean_meas[i] >=
                                                  trigger_level)
                if has_triggered:
                    n: int = measures_len
                    ...
                    for j in range(n):
                        if (measures[i, k] >=
                                trigger_level) and (j <= jTrigger + gap_len):
                            jZero = jTrigger = j
                    ...
                    num_measures_to_flush = (min(max(num_measures_to_flush, j),
                                                 n))
        ...
        if has_triggered:
            flushedLen_ns = (measures_len -
                             num_measures_to_flush) * measure_period_ns
